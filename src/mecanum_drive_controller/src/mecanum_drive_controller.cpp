  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  const double wheel_separation_w = params_.wheel_separation_width;
  const double wheel_separation_h = params_.wheel_separation_height;
  ...
  {
    double ff_feedback = registered_wheel_handles_[FL_WHEEL].feedback.get().get_value();
    double fr_feedback = registered_wheel_handles_[FR_WHEEL].feedback.get().get_value();
    double rf_feedback = registered_wheel_handles_[RL_WHEEL].feedback.get().get_value();
    double rr_feedback = registered_wheel_handles_[RR_WHEEL].feedback.get().get_value();
    if (params_.position_feedback)
    {
      odometry_.update(ff_feedback, fr_feedback, rf_feedback, rr_feedback, time);
    }
    else
    {
      odometry_.updateFromVelocity(
        ff_feedback * wheel_radius * period.seconds(),
        fr_feedback * wheel_radius * period.seconds(),
        rf_feedback * wheel_radius * period.seconds(),
  ...
  const double sum_separation = (wheel_separation_w + wheel_separation_h)/2.0;
  const double velocity_fl = inv_r*(linear_x_command - linear_y_command - sum_separation*angular_command);
  const double velocity_fr = inv_r*(linear_x_command + linear_y_command + sum_separation*angular_command);
  const double velocity_rl = inv_r*(linear_x_command + linear_y_command - sum_separation*angular_command);
  const double velocity_rr = inv_r*(linear_x_command - linear_y_command + sum_separation*angular_command);
  // Set wheels velocities:
  registered_wheel_handles_[FL_WHEEL].velocity.get().set_value(velocity_fl);
  registered_wheel_handles_[FR_WHEEL].velocity.get().set_value(velocity_fr);
  registered_wheel_handles_[RL_WHEEL].velocity.get().set_value(velocity_rl);
  registered_wheel_handles_[RR_WHEEL].velocity.get().set_value(velocity_rr);
  return controller_interface::return_type::OK;
}
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  const double wheel_separation_width = params_.wheel_separation_width;
  const double wheel_separation_height = params_.wheel_separation_height;
  const double wheel_radius = params_.wheel_radius;

  odometry_.setWheelParams(wheel_separation_width, wheel_separation_height, wheel_radius);
  ...
  limiter_linear_x_ = diff_drive_controller::SpeedLimiter(
  ...
  limiter_linear_y_ = diff_drive_controller::SpeedLimiter(
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  const std::vector<std::string> & wheel_names = {
    params_.front_left_wheel_name,
    params_.front_right_wheel_name,
    params_.rear_left_wheel_name,
    params_.rear_right_wheel_name
  };
  registered_wheel_handles_.reserve(4);
...
} //end namespace mecanum_drive_controller
